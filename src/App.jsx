...
import { initLeagueRosters, sortStandings, collectSeasonEndAchievements, processSeasonSwaps, initLeague, initAILeague, buildSeasonCalendar, initCup, advanceCupRound, buildNextCupRound, resolveKnockoutPromotion, collectDynastyCupFinalAchievements, collectMiniTournamentThirdPlaceAchievements, collectMiniTournamentFinalAchievements, getPriorPlayedResult } from "./utils/league.js";
import { accumulateMatchStats, accumulateCupMatch, makeCupAIMatchHandler, leagueMatchId, emptyCompetitionStats, cupKey as makeCupKey } from "./utils/competitionStats.js";
...
import { useSeasonEnd } from "./hooks/useSeasonEnd.js";
...
import { createUnlockablePlayer, checkAchievements, deriveMissingPlayerUnlocks, checkLegendMilestones, checkCheatingDeath, hasAllBackPages, addHatTrickScorer, collectBreakoutAchievements, isSignedFromRival, collectLineupAchievements, getFavouriteStartsIncrement } from "./utils/achievements.js";
...
function FruitCigs() {
  ...
  const {
    ...
    setOnboardingDripSuppressed, setOnboardingSilencedByChoice,
    setCompareSignWatch, setFanSentimentSeasonFloor, setUltimatumsSurvived, setLegendCarryCounts,
    setLatestHeadline,
    ...
    try {
      ...
      if (fanSnap != null) {
        ...
        if (holidayDelta !== 0) {
          ...
          if (!unlockedAchievements.has("riding_the_wave") && holidayDelta >= 20) tryUnlockAchievement("riding_the_wave");
        }
  ...
  const updateUltimatumProgress = useCallback((playerWon, isDraw, cupPlayerEliminated) => {
    ...
    if (newPts >= target) {
      ...
      if (!useGameStore.getState().unlockedAchievements.has("tightrope_walker")) tryUnlockAchievement("tightrope_walker");
      if (newGames >= 1 && !useGameStore.getState().unlockedAchievements.has("stay_of_execution")) tryUnlockAchievement("stay_of_execution");
      // Cheating Death — career-long, never reset at prestige (the Ironman
      // career continues through prestige, it doesn't restart).
      const newUltimatumsSurvivedM = useGameStore.getState().ultimatumsSurvived + 1;
      setUltimatumsSurvived(newUltimatumsSurvivedM);
      if (checkCheatingDeath(newUltimatumsSurvivedM, useGameStore.getState().unlockedAchievements)) tryUnlockAchievement("cheating_death");
  ...
  }, [setBoardWarnCount, setInboxMessages, triggerSacking, tryUnlockAchievement]);
  ...
  const handleInboxChoice = useCallback((msg, choice) => {
    ...
    if (msg.type === "asst_mgr_training_intro" || msg.type === "asst_mgr_training_nudge") {
      if (choice === "delegate") {
        ...
        // onboardingSilencedByChoice is distinct from onboardingDripSuppressed
        // above: that flag defaults true for old saves that never chose, so
        // it can't gate the "silence the tips" achievement trio. This one is
        // only ever set true at an explicit opt-out/drip_stop choice.
        setOnboardingSilencedByChoice(true);
      }
    }
    ...
    <div style={{
      ...
      {showAchievements ? (
        ...
        <TransfersPage
          ...
          setCompareSignWatch={setCompareSignWatch}
          ...
          onDone={(cupWasAlwaysFast, cupWasAlwaysNormal) => {
            ...
            if (newCupCalIdx >= cal2.length) {
              ...
              const newSeasonUnlocks2 = collectSeasonEndAchievements({
                ...
                gameMode,
                onboardingSilencedByChoice: useGameStore.getState().onboardingSilencedByChoice,
                compareSignWatch: useGameStore.getState().compareSignWatch,
                fanSentimentSeasonFloor: useGameStore.getState().fanSentimentSeasonFloor,
                seasonLeagueStatsAvailable: useGameStore.getState().seasonLeagueStatsAvailable,
                transferHistory, shortlist, dossierBurns: useGameStore.getState().dossierBurns,
                leagueHistory, teamName,
              }, BGM.getCurrentTrackId());
              // See the matching clear in useMatchResult.js — this is the
